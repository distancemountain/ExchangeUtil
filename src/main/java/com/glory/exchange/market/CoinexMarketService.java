...
//        	market.setAmount(null);
			...
			e.printStackTrace();
			super.logger.error(com.glory.common.Constants.EXCHANGE_NAME.EXCHANGE_COINEX +":"+ CoinexAPI.market_ticker +" 请求失败", e);
		}
    	...
    	try {
        	...
        	for (Map<String, Object> map : tickerList) {
	    		...
	    		if (Long.valueOf(item.get("date_ms").toString()).longValue() <= time) {
	    			...
	    			dealtInfo.setDirection(com.glory.common.Constants.TRADING_DIRECTION.valueOf(item.get("type").toString().toUpperCase()));
			...
			e.printStackTrace();
			super.logger.error(com.glory.common.Constants.EXCHANGE_NAME.EXCHANGE_COINEX +":"+ CoinexAPI.market_deals +" 请求失败", e);
		}
