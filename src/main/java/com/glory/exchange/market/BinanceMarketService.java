...
public class BinanceMarketService extends MarketService implements IMarketService {
	...
	public List<Market> quotationTicker(String accessKey, String secretkey, String... symbols) {
    	...
    	try {
			...
			e.printStackTrace();
			super.logger.error(com.glory.common.Constants.EXCHANGE_NAME.EXCHANGE_BINANCE +":"+ BinanceAPI.api_ticker_24hr +" 请求失败", e);
		}
	...
	//		}
	    	...
	    	for (Map<String, Object> map : tickerList) {
	    		...
	    		if (Long.valueOf(item.get("time").toString()).longValue() <= time) {
	    			...
	    			dealtInfo.setDirection((Boolean.valueOf(item.get("isBuyerMaker").toString()) ? com.glory.common.Constants.TRADING_DIRECTION.BUY : com.glory.common.Constants.TRADING_DIRECTION.SELL));
			...
			e.printStackTrace();
			super.logger.error(com.glory.common.Constants.EXCHANGE_NAME.EXCHANGE_BINANCE +":"+ BinanceAPI.api_trades +" 请求失败", e);
		}
